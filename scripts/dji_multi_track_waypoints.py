...
def track_waypoints():
    ...
    for uav_id in range(1,wp_data["uav_n"]+1):
        ...
        if(uav_id == 1):   
            rospy.wait_for_service(config[1])
        else:
            rospy.wait_for_service(config[0]+config[1])

        try: 
            if(uav_id == 1):
                get_auth = rospy.ServiceProxy(config[1], SetBool)
            else:
                get_auth = rospy.ServiceProxy(config[0]+config[1], SetBool)
            res = get_auth(True)
            if(True):
                print("Authority obtained")
                ...
                print("Error getting authority")
        except rospy.ServiceException as e:
            print("Control Authority service call failed: %s"%e)
